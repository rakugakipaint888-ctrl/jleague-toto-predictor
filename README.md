...
トリプルの基本買い目とします。引分候補閾値を超えただけで0を強制しません。
P(0)が上位2結果に入る場合はダブルに0を含め、3位なら閾値だけを理由に上位結果と
入れ替えません。
...
満たす場合に表示します。0の購入を義務化する判定ではありません。
...
Version7-C側では、Score重みの実績検証、買い目バックテスト標本の拡充、mini toto
払戻形式、組み合わせ出力のストリーミング化を引き継ぎます。
...
Uncertainty／Double／Triple Score、シングル信頼度、区分、買い目、Coverage、
判定理由を保存します。全組み合わせCSVは1口1行、対象側の試合番号順です。
