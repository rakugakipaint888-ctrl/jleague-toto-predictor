...
ユーザー向けの現在のApp Versionは **Version8** です。Version8-A／B／Cは、
Version8内の実戦履歴・モデル診断・改善提案という機能区分を表します。
...
- Version8-Cの根拠付き原因候補・改善候補・優先度・信頼度ランキング
- Version8-B診断に基づく再最適化の不要／検討／推奨／データ不足判定と提案履歴CSV
- 公式データ → CSV → 手入力の自動切替
...
公式開催回の13試合予測が成立した時点で一意な`prediction_run_id`を生成し、
その予測から作ったVersion7-C買い目と「実戦予測として保存」を同じrunへ紐付けます。
保存時には、公式開催回の13試合、予測日時、使用Version、フル精度P(1)・P(0)・P(2)、
本命、予想スコア、期待得点、Elo、
引分候補、引分候補閾値、設定Snapshotを保存します。後から現在モデルで過去確率を
再計算・上書きしません。

1回の予測には次の一意IDを付けます。同じ開催回で再予測した場合も別runとして
保存でき、同じ画面予測に対するStreamlit再実行や保存ボタン連打は同じrunへ束ねます。
...
Version8-CはVersion8-Bが確定した`DiagnosticReport`を主入力にし、検知済みの事実と
数値を、診断、原因候補、改善候補、優先度、信頼度、再最適化推奨度の順で表示します。
Version8-Aの生データを現在モデルで再予測せず、Version8-Bの診断式も複製しません。
「モデル診断」タブで診断を実行した後、「AI改善提案」タブの「改善提案を生成」を
押した場合だけ提案を構造化します。
...
根拠表には現在値、基準値、差、診断、根拠となったVersion8-B異常codeを表示します。
同じ原因・検証先を持つBrier、Log Loss、Calibrationの同時悪化は「モデル確率精度の
再検証」へ統合し、提案を重複させません。提案は優先度、信頼度の順にランキングし、
存在する件数だけ表示します。件数合わせの提案は生成しません。

### 提案カテゴリと原因・改善候補

ルールは`improvement_config.py`と`improvement_recommendations.py`へ集約しています。
モデル全体、引分性能、1予測、2予測、Calibration、Brier Score、Log Loss、
リーグ別性能、買い目戦略、Coverage、データ品質、再最適化タイミングを扱います。

- 引分：実引分率、本命0率、平均P(0)、Recall、F1、Brier、Calibration、AI推奨の
  0採用率・実引分カバー率を結び付ける。P(0)が実引分率に近いのに本命0率だけ低い、
  0を買い目へ入れてもカバーできない状態を区別する
- Calibration：Version8-B確率帯表から最大乖離帯を選び、過信または過小評価の可能性を
  示す。新しいCalibration方式は導入しない
- Brier／Log Loss：確率精度の悪化、誤った高信頼予測の可能性としてまとめ、結果別・
  リーグ別の偏り確認や既存Version7-B探索範囲での再検証を候補にする
- リーグ別：Version8-Bが性能差を検知したJ1/J2/J3だけを分離し、当該リーグに限定した
  誤り分布確認・既存バックテストを候補にする。リーグ別モデルは作成しない
- 1／0／2：Version8-Bの結果別Recall異常とPrecision、F1、Brier、Calibrationを根拠に、
  該当クラスだけを優先確認する。0は引分カテゴリで独立表示する
- 買い目／Coverage：AI推奨と実購入を混ぜず、平均Coverage、Coverage帯別完全カバー率、
  口数差、simulation／actual ROI差を使う。ダブル・トリプル配分、引分優先順位、
  保存済み戦略バックテストの再確認を候補にする
- データ品質：Version8-Bが検知した欠損・不正値・重複と除外件数をモデル性能と分離し、
  モデル設定より先に保存経路の確認を候補にする

原因は断定せず、数値だけで特定できない場合は「原因は診断結果だけでは特定できません」
と表示します。係数や閾値の具体的な変更量は生成しません。

### 優先度・信頼度・再最適化推奨度

優先度と信頼度はLLMの主観ではなく固定スコアです。初期値は
`ImprovementThresholds`で一元管理し、自動学習・自動変更しません。

| 判定 | 固定ルール |
|---|---|
| 優先度 | 最大異常レベルを情報=1、注意=2、警告=3とし、複数指標一致とモデル全体への影響を各+1。4以上=高、2以上=中、それ以外=低 |
| 信頼度 | Version8-B最低基準充足=1、65試合かつ5開催=2、警告・複数指標・直近5/10一致を各+1。4以上=高、2以上=中、それ以外=低 |
| 再最適化：不要 | データ十分で、性能警告なし・性能注意2件未満 |
| 再最適化：検討 | 性能警告1件、または性能注意2件以上 |
| 再最適化：推奨 | 性能警告2件以上、または異常系統3種類以上 |
| 再最適化：データ不足 | Version8-Bがデータ不足と判定 |

買い目補助ルールの初期値は、評価済み買い目2件以上、低Coverage 5%未満、Coverage帯の
完全カバー率改善5pt未満、AI推奨／実購入Coverage差5pt以上、平均口数差2口以上、
実購入ROI 1.0未満、simulation／actual ROI差20pt以上です。引分補助ルールは、
平均P(0)と実引分率の近さ5pt以内、AI推奨0採用率と本命0率の差10pt以上、実引分カバー率
50%未満です。Coverageは実際の当選確率として扱いません。

Version8-Bの最低26試合かつ2開催を満たさない場合、性能・買い目の改善候補を無理に
生成せず、再最適化は「データ不足」、「データ不足のため設定変更は推奨しません」を
主表示します。データ品質問題は少数データでも別枠で確認できます。

### AI整文・フォールバック・提案履歴

提案の判断は常に固定ルールで先に構造化します。任意の文章formatterを接続する場合も、
AIへ渡すのは構造化済み説明だけです。AIが新しい数値を追加した、変更を実施したと表現した、
空文字・不正型・長すぎる文章を返した、またはAIを利用できない場合は、その提案だけ
テンプレート文章へフォールバックします。外部AI接続は起動・診断・提案生成の必須条件では
なく、標準構成は再現可能なテンプレートモードです。

生成結果はVersion8-A/Bとは別の次の実行時CSVへ不変hash付きで原子的に保存します。

```text
data/history/model_improvement_history.csv
```

提案日時、提案ID、診断run ID、対象期間・リーグ・Version、試合・開催数、検知異常、
提案カテゴリ、現在値・基準値・差、原因候補、改善候補、優先度・信頼度と各根拠、
再最適化推奨度、文章モードを保持し、画面からCSVを取得できます。

### 自動変更禁止とVersion8完成範囲

Version8-Cには設定適用・最適化実行ボタンがありません。`config.py`や採用済みsettings、
モデル、閾値、Coverage、買い目ルールを書き換えず、Version7-B／Optunaを起動せず、
モデルを自動採用しません。Version8の完成範囲は、Version8-Aの不変な実戦保存、
Version8-Bの事実・診断・異常検知、Version8-Cの根拠付き検証候補提示までです。
最終判断と実行はユーザーが行います。

既知の制限は次のとおりです。

- 改善提案は同じStreamlit sessionで最後に実行したVersion8-B診断を入力にする。
  診断履歴CSVだけから詳細`DiagnosticReport`を復元する機能はない
- 因果関係を証明せず、原因候補は可能性としてのみ表示する
- 外部AI APIのclientや認証は同梱せず、標準はテンプレート文章。任意formatterを接続しても
  新しい数値・自動適用表現は拒否する
- Version8-Bに存在しない特徴量別・対戦条件別の根拠は推測生成しない
- サンプル基準と提案閾値は説明可能な初期値であり、統計的最適値ではない
- 実行時提案履歴はGit管理外であり、別環境へ自動同期しない
...
├── improvement_config.py     # Version8-Cの提案・優先度・信頼度ルール
├── improvement_recommendations.py # 診断結果から原因・改善候補を構造化
├── improvement_history.py    # 改善提案履歴専用CSV・原子的保存
├── improvement_ui.py         # AI改善提案タブ・ランキング・注意事項
├── data/
...
│   ├── test_improvement_recommendations.py # Version8-Cルール・履歴・不変性
...
- Version8-C：診断結果を根拠にした原因候補・改善候補・優先度・信頼度・提案履歴
